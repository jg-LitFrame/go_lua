...
func (lexer *Lexer) skipWhiteSpace() {
	for len(lexer.chunk) > 0 {
		if lexer.test("--") {
			...
			lexer.next(2)
			lexer.line++
		} else if lexer.isNewLine(lexer.chunk[0]) {
			lexer.next(1)
			lexer.line++
		} else if lexer.isWhiteSpace(lexer.chunk[0]) {
			lexer.next(1)
		} else {
			break
		}
...
func (lexer *Lexer) isNewLine(c byte) bool {
	return c == '\r' || c == '\n'
}
...
func (lexer *Lexer) skipComment() {
	...
	for len(lexer.chunk) > 0 && !lexer.isNewLine(lexer.chunk[0]) {
		lexer.next(1)
	}
}
...
func (lexer *Lexer) escape(str string) string {
	...
	for len(str) > 0 {
		...
		case 'z':
			str = str[2:]
			for len(str) > 0 && lexer.isWhiteSpace(str[0]) { // todo
				str = str[1:]
			}
			continue
		}
